        ...
        .then(function(result){
            ...
            session
                ...
                .then(function(result2){
                    ...
                    session
                        .run("MATCH (n:Interest) RETURN n")
                        .then(function(result3){
                            var interestArr = [];
                            result3.records.forEach(function(record){
                                interestArr.push(record._fields[0].properties);
                            });
...
app.get('/graph.html', function(req, res){
    /*data = fs.readFile('/graph.html', function (err, data){
        res.setHeader('Content-Type', 'text/html');
        res.send(data);
    })*/
    //return res.redirect('/graph.html');
...
app.post('/person/interest/add', function(req, res){
    ...
    session
        .run("CREATE(n:Interest {name:{nameParam}}) RETURN n.name", {nameParam: interest})
        .then(function(result){
            res.redirect('/');
            session.close();
        })
